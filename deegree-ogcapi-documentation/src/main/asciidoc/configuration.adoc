...
<2> the workspace root directory, the exact name is required, path can be set with environment variable `DEEGREE_WORKSPACE_ROOT` (mandatory)
...
<6> Provider license applicable to the service provider with description element
<7> Dataset license applicable to the dataset using link to license element
