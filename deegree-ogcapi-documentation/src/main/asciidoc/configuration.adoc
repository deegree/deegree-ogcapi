...
<2> the workspace root, exact name is required, path can be set with environment variable `DEEGREE_WORKSPACEROOT` (mandatory)
...
<16> global metadata configuration (optional)
...
The configuration file can contains the following elements:
...
Each dataset is configured in a separate file. This example shows a minimal configuration for a dataset called "streets". The filename defines the _{datasetId}_.
...
<9> Metadata link in format `application/xml`
<10> Metadate link in format `text/html`

NOTE: The dataset configuration file shall be stored in the subdirectory _ogcapi/_. The file is mandatory.
...
|ServiceMetadata |0..1 |Complex |Configuration of service and dataset metadata
|===
...
To configure the HTML encoding the following configuration file can be used.
...
NOTE: The file _treesview.xml_ shall be stored in the subdirectory _html/_. To define a global configuration the file name must be _htmlview.xml_. The file is optional.
