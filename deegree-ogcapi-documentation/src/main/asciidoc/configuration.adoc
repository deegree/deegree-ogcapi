...
===== Using schema-driven FeatureStore configurations

When using an GML application schema in the FeatureStore configuration the schema is used in the OpenAPI document for XML and JSON encoding to describe the data types. Is no application schema provided
the data type description is derived from the FeatureStore mapping (table-driven mode).
...
NOTE: When using the schema-driven mode always copy the GML application schema into the `appschemas/` directory of the workspace and reference the schema in the FeatureStore configuration.
