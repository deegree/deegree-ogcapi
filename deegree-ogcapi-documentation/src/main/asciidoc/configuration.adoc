...
<10> Metadata link in format `application/xml` for the dataset.
<11> Metadata link in format `text/html` for the dataset.
...
<2> Service metadata link, in the context of OAF this link is used in the collection view link of the metadata
<3> Service metadata link in format `text/html`, in the context of OAF this link is used in the collection view linking to the HTML representation of the metadata
<4> Feature collection name which links to the feature type configured, here the {collectionId}
...
<HtmlView xmlns="http://www.deegree.org/ogcapi/htmlview"
  ...
  <Map>
    <WMSUrl version="1.3.0">https://geodienste.hamburg.de/HH_WMS_Cache_Stadtplan</WMSUrl> <!--5-->
    ...
    <Source><![CDATA[© <a href="https://www.hamburg.de/" target="_new">Datenquelle</a>]]></Source> <!--8-->
  </Map>
...
<8> source of the map, can contain HTML in a CDATA tag
