...
existing workspace concept using the API of deegree webservices. With deegree OAF spatial data can be served with a resource-oriented approach.
...
. Install deegree OAF webapp and start the runtime environment. This is described in section <<installation>>.
...
=== feature store
Is a ressource within the deegree workspace providing access to a feature collection. In the context of deegree OAF a feature store provides access to datasource such as SHAPE file or a database.
