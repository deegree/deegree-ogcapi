...
/**
 * Filter that provides OpenApi endpoints under the alias "openapi" in addition to the
 * original "api".
 */
@Provider
@PreMatching
public class OpenApiAliasFilter implements ContainerRequestFilter {
	...
	public void filter(ContainerRequestContext requestContext) throws IOException {
		...
		// if the relative path is /datasets/{dataset}/openapi* replace "openapi" with
		// "api"
		if (segments.size() == 3 && segments.get(2).getPath().startsWith("openapi")
