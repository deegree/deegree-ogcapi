...
public class DeegreeQueryBuilder {
	 ...
	 * @param featureTypeMetadata the feature type metadata, never <code>null</code>
	 * @param featuresRequest the request to map, never <code>null</code>
	 * @return the created {@link Query}, never <code>null</code>
	 * @throws InternalQueryException if the featuresRequest could not be mapped
	 */
	...
	private ComparisonOperator createFilterOperator(FilterProperty filterProperty, String value) {
		...
		if (value != null && value.startsWith(">=")) {
			...
			return new PropertyIsGreaterThanOrEqualTo(valueReference, literal, true, ANY);
		}
		else if (value != null && value.startsWith("<=")) {
			Literal literal = new Literal(value.substring(2));
			return new PropertyIsLessThanOrEqualTo(valueReference, literal, true, ANY);
		}
		else if (value != null && value.startsWith(">")) {
			Literal literal = new Literal(value.substring(1));
			return new PropertyIsGreaterThan(valueReference, literal, true, ANY);
		}
		else if (value != null && value.startsWith("<")) {
			Literal literal = new Literal(value.substring(1));
			return new PropertyIsLessThan(valueReference, literal, true, ANY);
		}
		else if (value != null && value.contains("*")) {
			Literal literal = new Literal(value);
			return new PropertyIsLike(valueReference, literal, WILD_CARD, SINGLE_CHAR, ESCAPE_CHAR, false, ANY);
		}
		Literal literal = new Literal(value);
		return new PropertyIsEqualTo(valueReference, literal, false, ANY);
	}
	...
	private Operator createDatetimeOperator(Expression dateProperty, String datetime) throws InvalidParameterValue {
	...
	private Envelope asEnvelope(FeaturesRequest featuresRequest) throws InternalQueryException {
		...
		return simpleGeometryFactory.createEnvelope(bbox.get(0), bbox.get(1), bbox.get(2), bbox.get(3), crs);
	}
	...
	private class DatetimeInterval {
		...
		private DatetimeInterval(String datetime) throws InvalidParameterValue {
