...
public class ExceptionMediaTypeUtil {
	...
	/**
	 * Selects the best matching media types (application/json or application/xml)
	 * supporting exception messaged by the requested media type.
	 * @param request never <code>null</code>
	 * @return the best matching media type (application/json or application/xml), never
	 * <code>null</code>
	 */
	public static MediaType selectMediaType(Request request) {
		List<Variant> variants = Variant
			.mediaTypes(APPLICATION_JSON_TYPE, APPLICATION_OPENAPI_TYPE, APPLICATION_GEOJSON_TYPE, APPLICATION_XML_TYPE,
					APPLICATION_GML_TYPE, APPLICATION_GML_32_TYPE, APPLICATION_GML_SF0_TYPE, APPLICATION_GML_SF2_TYPE)
			.add()
			.build();
